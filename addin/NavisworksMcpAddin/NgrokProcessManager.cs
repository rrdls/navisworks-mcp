...
public sealed class NgrokProcessManager
{
    ...
    public string Status
    {
        get
        {
            ...
            if (StartedByAddin)
            {
                return "Public URL is running from this Navisworks session.";
            }
    ...
    public McpProcessResult Start()
    {
        lock (_lockObject)
        {
            ...
            {
                return McpProcessResult.Success("Public URL is already running from this Navisworks session.");
            }

            if (!McpProcessManager.PortIsOpen("127.0.0.1", NavisworksMcpRuntime.HttpPort))
            {
            ...
            {
                return McpProcessResult.Failure("Could not find ngrok.exe. Rebuild the release package or place ngrok.exe in %LOCALAPPDATA%\\NavisworksMcp\\app.");
            }

            CleanupStaleRuntimeProcess(ngrokPath);

            var domain = NavisworksMcpRuntime.NormalizeDomain(settings.NgrokDomain);
            WriteNgrokConfig(settings.NgrokAuthToken);

            var startInfo = new ProcessStartInfo
            {
                FileName = ngrokPath,
                Arguments = $"http {NavisworksMcpRuntime.HttpPort} --url=https://{domain} --config \"{NavisworksMcpRuntime.NgrokConfigPath}\"",
            ...
            try
            {
                ...
                return McpProcessResult.Success("Public URL started: " + NavisworksMcpRuntime.BuildPublicMcpUrl(settings));
            }
            catch (Exception ex)
            {
                process.Dispose();
    ...
    public McpProcessResult Stop()
    {
        lock (_lockObject)
        {
            if (_process is null || _process.HasExited)
            {
                ...
                ClearNgrokRuntimeState();
                return McpProcessResult.Success("No public URL process started by this Navisworks session was found.");
            }
            ...
            try
            {
                ...
                _process = null;
                ClearNgrokRuntimeState();
                McpLog.Info("Stopped ngrok process started by this Navisworks session.");
        ...
        lock (_lockObject)
        {
            if (_process is { HasExited: false })
            {
                try
                {
                    ...
                    McpLog.Info("Stopped ngrok during Navisworks shutdown.");
                }
                catch (Exception ex)
                {
                    McpLog.Error("Could not stop ngrok during Navisworks shutdown.", ex);
                }
                finally
                {
                    _process.Dispose();
    ...
    private static string? FindNgrokExecutable()
    {
        ...
        var candidates = new[]
        {
            Path.Combine(localAppData, "NavisworksMcp", "app", "ngrok.exe"),
    ...
    {
        Directory.CreateDirectory(NavisworksMcpRuntime.AppDataDirectory);
        var lines = new[]
        {
            "version: 3",
            "agent:",
        ...
        File.WriteAllLines(NavisworksMcpRuntime.NgrokConfigPath, lines);
    }
